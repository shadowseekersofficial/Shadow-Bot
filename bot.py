...
async def update_live_board(guild: discord.Guild):
    ...
    for uid, sess in sorted(sessions.items(), key=lambda x: x[1].get("start_time", 0)):
        ...
        # Grab avatar for the first operative (shown as thumbnail)
        if not first_avatar_url:
            try:
                member_obj = guild.get_member(int(uid))
                if member_obj and member_obj.display_avatar:
                    first_avatar_url = member_obj.display_avatar.url
            except Exception:
                pass

        # Per-operative avatar as a small icon via set_author isn't possible for multi-user
        # Instead embed each with their mention so Discord shows avatars inline
        member_obj = guild.get_member(int(uid))
        mention_str = member_obj.mention if member_obj else codename

        lines.append(
            f"{type_icon} **{codename}** {mention_str}{vc_badge}\n"
            f"┕ *{task}*\n"
            f"┕ `[{bar}]` {time_str}"
        )
    ...
    if first_avatar_url:
        embed.set_thumbnail(url=first_avatar_url)
